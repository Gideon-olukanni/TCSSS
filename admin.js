...
function setPhotoPreview(elId, url) {
  const el = $(`#${elId}`);
  if (!el) return;
  if (url) {
    el.src = url;
...
function formatPrincipalRange(item) {
  const start = escapeHtml(item.yearStarted || "?");
  if (item.isCurrent) return `${start} – Till Date`;
  return `${start} – ${escapeHtml(item.yearEnded || "?")}`;
}

function updatePrincipalYearEndedVisibility() {
  const isCurrent = $("#principalCurrent").checked;
  const field = $("#principalYearEndedField");
  const input = $("#principalYearEnded");
  if (!field || !input) return;
  field.hidden = isCurrent;
  input.required = !isCurrent;
  if (isCurrent) input.value = "";
}

// ============================================================
// Temporary diagnostic logger — writes a timestamped line to the
// visible debug panel AND the browser console, so the exact point
// where the login sequence stops is unambiguous on the next test.
// ============================================================
...
function clearPrincipalForm() {
  ...
  updatePrincipalYearEndedVisibility();
}
...
async function loadLeadership() {
  ...
  state.photoCache.ldPrincipal = data.principalPhoto || "";
  state.photoCache.ldBoy = data.seniorPrefectBoyPhoto || "";
  state.photoCache.ldGirl = data.seniorPrefectGirlPhoto || "";
  setPhotoPreview("ldPrincipalPhotoPreview", state.photoCache.ldPrincipal);
  setPhotoPreview("ldBoyPhotoPreview", state.photoCache.ldBoy);
  setPhotoPreview("ldGirlPhotoPreview", state.photoCache.ldGirl);
}
...
async function saveLeadership(event) {
  ...
  try {
    ...
    const payload = {
      ...
      principalPhoto: principalPhoto || state.photoCache.ldPrincipal || "",
      seniorPrefectBoyName: $("#ldBoyName").value.trim(),
      seniorPrefectBoyMessage: $("#ldBoyMsg").value.trim(),
      seniorPrefectBoyPhoto: boyPhoto || state.photoCache.ldBoy || "",
      seniorPrefectGirlName: $("#ldGirlName").value.trim(),
      seniorPrefectGirlMessage: $("#ldGirlMsg").value.trim(),
      seniorPrefectGirlPhoto: girlPhoto || state.photoCache.ldGirl || ""
    };

    await setDoc(documentRef(COLLECTIONS.leadership.collection, COLLECTIONS.leadership.doc), payload);

    state.photoCache.ldPrincipal = payload.principalPhoto;
    state.photoCache.ldBoy = payload.seniorPrefectBoyPhoto;
    state.photoCache.ldGirl = payload.seniorPrefectGirlPhoto;
...
async function loadPrincipals() {
  ...
  renderList("principalList", items, "No principal records yet.", (item) => {
    ...
    const subtitle = formatPrincipalRange(item);
    return createRow({
      thumb: item.photoUrl || "",
      title,
      subtitle,
...
async function savePrincipals(event) {
  ...
  try {
    ...
    if (!payload.isCurrent && !payload.yearEnded) throw new Error("Year ended is required for a past principal.");
...
async function loadAllData() {
  debugLog("loadAllData() started — kicking off 7 Firestore reads...");
  const tasks = [
    loadNews(),
    loadCouncil(),
    loadLeadership(),
  ...
  debugLog(`loadAllData() SETTLED — ${results.filter(r => r.status === "fulfilled").length}/7 succeeded`);
  const failed = results.filter((result) => result.status === "rejected");
  if (failed.length) {
    failed.forEach((f, i) => debugLog(`  Task ${i} rejected: ${f.reason?.message || f.reason}`));
...
$("#loginForm").addEventListener("submit", async (event) => {
  event.preventDefault();
  debugLog("Form submitted — preventDefault called");
  ...
  try {
    ...
    debugLog("signInWithEmailAndPassword RESOLVED — waiting for onAuthStateChanged...");
  } catch (error) {
    const message = error?.message || "Could not sign in.";
    debugLog(`signInWithEmailAndPassword REJECTED — ${error?.code || ""} ${message}`);
...
$("#principalCurrent").addEventListener("change", updatePrincipalYearEndedVisibility);
...
// This was missing before: unhandledrejection only catches async promise
// errors. A plain synchronous error (e.g. a typo'd DOM selector) would
// previously fail completely silently — no message, no dashboard, no clue.
window.addEventListener("error", (event) => {
  debugLog(`SYNCHRONOUS ERROR — ${event.message} (${event.filename}:${event.lineno})`);
  showStatus(event.message || "An unexpected error occurred.", "error");
});

onAuthStateChanged(auth, (user) => {
  debugLog(`onAuthStateChanged fired — user: ${user ? user.email : "null"}`);
  if (!user) {
    state.user = null;
    setView(false);
    return;
  ...
  setView(true);
  debugLog("setView(true) called — dashboard should now be visible");
  setAuthenticatedUser(user);
  setTab("news");
  debugLog("Calling loadAllData()...");
  void loadAllData();
});
